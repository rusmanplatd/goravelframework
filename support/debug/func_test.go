...
func TestFuncInfo(t *testing.T) {
	var f = func() {}
	info := GetFuncInfo(f)
	assert.Equal(t, "github.com/rusmanplatd/goravelframework/support/debug.TestFuncInfo.func1", info.Name)
	assert.Equal(t, "TestFuncInfo.func1", info.ShortName())
	assert.Equal(t, "github.com/rusmanplatd/goravelframework/support/debug", info.PackagePath())
	...
	assert.Equal(t, "github.com/rusmanplatd/goravelframework/support/debug.Dump", info.Name)
